      ...
      <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-12 customeTopBar width100">
                <div class="headItems">
                  <h1>Interests for #{{$liability->title}} ({{ number_format($interests->sum('amount'),2)}})</h1>
                </div>
                <div class="rhsItms">
                  ...
                  <div class="card-body">
                    <table class="table table-bordered">
                      ...
                      <tbody>
                          ...
                          @foreach ($interests as $payment)
                          <tr>
                            <td>{{$loop->iteration}}</td>
                            <td>{{date('d-M-Y, l',strtotime($payment->date))}}</td>
                            <td>{{$payment->title}}</td>
                            <td>{{number_format($payment->amount,2)}}</td>
                            <td><a class="btn btn-sm btn-danger" href="{{route('liabilities.interest.delete',encrypt($payment->id))}}">Delete</a></td>
                          <tr>
                          @endforeach
                      </tbody>
                    </table>
...
<div class="modal fade" id="modal-default">
    <div class="modal-dialog">
      <div class="modal-content">
          <form action="{{route('liabilities.interest.save')}}" method="post">
              @csrf
